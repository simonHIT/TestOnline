	...
	return this.each( function() {
		...
		constructTr = function(id, hide, altClass, rd, cur, selected) {
			var tabindex = '-1', restAttr = '', attrName, style = hide ? 'display:none;' : '',
				classes = 'ui-widget-content jqgrow qfd_padding_white ui-row-' + ts.p.direction + (altClass ? ' ' + altClass : '') + (selected ? 'qfd_padding_white1 ui-state-highlight' : ''),
			...
			if(pgboxes) {
				...
				if (ts.p.viewrecords){
					if(ts.p.reccount === 0) {
						...
						if(ts.p.noDataTip === true){
							$(".ui-jqgrid-nodata."+this.id).html($.jgrid.getRegional(ts, "defaults.emptyrecords", ts.p.emptyrecords )).css("display","block");
						}
					} else {
						from = base+1;
						tot=ts.p.records;
						...
						$(".ui-jqgrid-nodata."+this.id).css("display","none");
					}
				}
						...
						if (i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled') && !$(this).hasClass("jqfoot")){
								...
								if(froz) {
									...
									$("#"+$.jgrid.jqID(this.id), ts.grid.fbDiv).addClass("ui-state-highlight qfd_padding_white1");
								}
							}
						}
					});
		...
		$(grid.bDiv)
			...
			.append($("<div style='display: none;'></div>").addClass("ui-jqgrid-nodata").addClass(this.id))
		...
		var arf = ts.p.hidegrid===true ? $("<a role='link' class='ui-jqgrid-titlebar-close displaynone ui-corner-all HeaderButton' title='"+($.jgrid.getRegional(ts, "defaults.showhide", ts.p.showhide) || "")+"'" + " />").hover(
			function(){ arf.addClass('ui-state-hover');},
			function() {arf.removeClass('ui-state-hover');})
		.append("<span class='displaynone ui-icon ui-icon-circle-triangle-n'></span>").css((dir==="rtl"?"left":"right"),"0px") : "";
		$(grid.cDiv).append(arf).append("<span class='ui-jqgrid-title qfd_padding_color'>"+ts.p.caption+"</span>")
		.addClass("ui-jqgrid-titlebar qfd_padding ui-jqgrid-caption"+(dir==="rtl" ? "-rtl" :"" )+" ui-widget-header ui-corner-top border-none ui-helper-clearfix ");
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						.addClass("ui-state-default ui-th-column-header qfd_padding_bg ui-th-"+ts.p.direction)
					...
					if (skip === 0) {
						if (o.useColSpanStyle) {
							...
							//2017-07-04 xwj为实现动态表多层表头合并，对此代码进行扩展
