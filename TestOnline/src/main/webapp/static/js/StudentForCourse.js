...
function getCourses(){
	...
	$.ajax({
		type : "GET",
		url : "../TestOnline/course/stuCourse?userId="+userId,
		async : true,
		dataType : 'json',
		cache : false,
		success : function (reg){
			...
			if(status.code == 0){
				for(var i = 0; i < dataList.length ; i++){
					str += "<div class = course_block ><div class = course_img style='background: url(../TestOnline/upload/" + dataList[i].courseImg +") no-repeat; background-size: 100%;'> </div> <a href=\"CourseInfo.html?courseId="+ dataList[i].courseId +"\">" + dataList[i].courseName + "</a></div>";
				}
